...
- [x] 1.1 `npx nx test baza-frontend --testFile=route-map-arrow.spec.ts --testFile=route-map-geometry.spec.ts` passes
- [x] 1.2 `npx nx test baza-frontend` passes
- [x] 1.3 `npx nx lint baza-frontend` passes
- [x] 1.4 Root `package.json` has no `leaflet-polylinedecorator` / `@types/leaflet-polylinedecorator`

#### Manual

- [x] 1.5 On offer detail (desktop), zoom the map: arrows stay on the dashed lines
- [x] 1.6 On `/offers`, the filter row uses full list width and does not overflow
- [x] 1.7 Hover a route in the detail list: that hop is emphasized, others dim; leave restores
