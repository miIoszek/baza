...
Professional driver. Browses a job listing plus interactive map, sees route visualization in job details, and applies by signing / submitting a CV without creating an account in MVP. Drivers are the supply side; the MVP is built so companies retain drivers, not so drivers are the paying customer. Driver accounts and saved profiles (experience, qualifications, route preferences, looking vs open to offers) are deferred beyond MVP.
...
1. **Hard deadline vs estimate** — Hard deadline 2026-09-12 is tighter than the stated 2-week after-hours estimate (~9 calendar days remaining at shape completion). Owner: user. Resolve: move deadline, cut scope, or accept slip. Block: planning risk.
2. **Company login mechanism** — Login is required for companies, but email+password vs OAuth vs passwordless was not locked. Owner: user / tech-stack step. Block: no.
3. **Primary company role** — Hiring decision-maker role remains TBD (recruiter / fleet manager / owner). Owner: user. Block: no for MVP.
4. **Application “message” field** — FR-005 lists contact information, message, and CV in the inbox; FR-009 / US-01 apply form only specifies email, phone, and CV. Is an optional message part of apply? Owner: user. Block: no.
5. **Driver license category in MVP filters** — Business logic mentions license category “when available”; MVP has no driver profile. Is license a Job Offers filter in MVP or deferred? Owner: user. Block: no.
